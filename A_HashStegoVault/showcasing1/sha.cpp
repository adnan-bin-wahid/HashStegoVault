...
{
    // Input file path
    std::string filePath = "input.txt";  // Replace with the actual file path
    ...
    // Save the hash in hash.txt
    std::ofstream hashFile("hash.txt");
    if (!hashFile.is_open()) {
        std::cerr << "Error opening hash.txt for writing." << std::endl;
        return 1;
    }
    ...
    std::cout << "Hash saved in hash.txt" << std::endl;
